...
//        currentOrientation = convertOrientation(UIDevice.currentDevice().orientation)
        ...
        switch optimizeFor {
        case .BatterySaving : faceDetectorOptions = [CIDetectorAccuracy : CIDetectorAccuracyLow, CIDetectorTracking: true]
        case .HigherPerformance : faceDetectorOptions = [CIDetectorAccuracy : CIDetectorAccuracyHigh, CIDetectorTracking: true]
        }
        
        self.faceDetector = CIDetector(ofType: CIDetectorTypeFace, context: self.ciContext, options: faceDetectorOptions)
    }
